...
class RuntimeContextTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_no_project_matches_env_floor(self):
        ...
        ctx = await resolve_runtime_context(
            self.store, self.settings, user_id="u", user_key="default"
        )
    ...
    async def test_project_config_overrides_floor(self):
        ...
        await self.store.upsert_project_config(
            proj["id"], model_name="gemini-2.5-flash", rag_top_k=7
        )
        ctx = await resolve_runtime_context(
    ...
    async def test_signature_changes_with_model(self):
        ...
        c1 = await resolve_runtime_context(
            self.store, self.settings, user_id=user["id"], user_key="uk", project_id=p1["id"]
        )
        c2 = await resolve_runtime_context(
            self.store, self.settings, user_id=user["id"], user_key="uk", project_id=p2["id"]
        )
