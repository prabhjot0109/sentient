...
class _FakeEmbeddings(Embeddings):
    def embed_query(self, text: str) -> list[float]:
        low = text.lower()
        return [
            float(sum(ord(c) for c in low) % 997),
            float(len(low)),
            float(low.count("guard") * 10),
        ]
...
class FaissBackendTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_index_then_retrieve_roundtrip(self):
        ...
        await backend.index(
            [
                Document(
                    page_content="Sentinel guards the archives.",
                    metadata={"source": "lore.txt", "chunk_id": 0},
                ),
                Document(
                    page_content="Nords have frost resistance.",
        ...
        results = await backend.retrieve(
            "who guards the archives", k=2, search_type="similarity", min_score=0.0
        )
    ...
    async def test_remove_source_drops_chunks(self):
        ...
        got = {
            d.metadata.get("source") for d, _ in await backend.retrieve("x", k=10, min_score=0.0)
        }
